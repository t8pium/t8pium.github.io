...
BASE=os.environ.get('FVG_ACTIVE_PKL','data/active_mnq.pkl')
OUT=Path(os.environ.get('FVG_OUTPUT_DIR','results/body_acceptance')); OUT.mkdir(parents=True,exist_ok=True)
...
base_h=a.high.to_numpy(float); base_l=a.low.to_numpy(float)
...
def detect_fvgs(b):
    ...
    w=up-lo
    ok=w>=0.25-1e-12
    return ix[ok],d[ok],lo[ok],up[ok],w[ok],sym
...
for tf in TFS:
    ...
    for mode,trig,depth,ag in [('qualifying',tq,dep,age),('first_touch',tt,depf,agef)]:
        ...
        ts=b.end_ts.to_numpy()[tr]; formts=b.end_ts.to_numpy()[fi]
    ...
    all_events.append(pd.DataFrame({'timeframe':TFNAME[tf],'tf_min':tf,'formation_ts':b.end_ts.to_numpy()[ix],
        'direction':np.where(d==1,'bullish','bearish'),'lower':lo,'upper':up,'width_pts':w,'width_ticks':w/.25,
...
trades.to_pickle(OUT/f'trades{suffix}.pkl'); events.to_pickle(OUT/f'fvg_events{suffix}.pkl')
if os.environ.get('CE_CHUNK')=='1':
    print('chunk saved', suffix, len(trades), len(events), flush=True); raise SystemExit
...
def agg(df,groupcols):
    zr=df[df.outcome_code.isin([1,-1,2])].copy()
    ...
    out=out.merge(be,on=groupcols,how='left'); out['edge_vs_breakeven_pp']=100*(out.win_rate-out.avg_breakeven_winrate)
    return out

by_tf=agg(primary,['timeframe','tf_min']).sort_values('tf_min'); by_depth=agg(primary,['depth_bucket'])
...
def boot_metric(z,B=1000):
    ...
    arr=(z.outcome_code.to_numpy()==1).astype(float); r=z.realized_R_conservative.to_numpy(float); n=len(z)
    wr=np.empty(B); er=np.empty(B)
    for b in range(B):
        ii=rng.integers(0,n,n); wr[b]=arr[ii].mean(); er[b]=np.nanmean(r[ii])
    return (*np.quantile(wr,[.025,.975]),*np.quantile(er,[.025,.975]))
ci=[]
for bucket in labels:
    z=primary[primary.depth_bucket.astype(str)==bucket]; wlo,whi,rlo,rhi=boot_metric(z,500); ci.append([bucket,len(z),wlo,whi,rlo,rhi])
wlo,whi,rlo,rhi=boot_metric(ce,1000);ci.append(['45-55%',len(ce),wlo,whi,rlo,rhi])
pd.DataFrame(ci,columns=['slice','N','win95_low','win95_high','meanR95_low','meanR95_high']).to_csv(OUT/'bootstrap_ci.csv',index=False)
...
summary={'data_start':str(a.ts_event.min()),'data_end':str(a.ts_event.max()),'base_rows':len(a),
